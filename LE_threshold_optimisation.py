...
def leading_edge_time(wf: np.ndarray, pk: int, thr: float, dt: float, search_back: int = 80, 
    ...
    # Search backwards: find last k where wf[k-1] > level and wf[k] <= level
    for k in range(hi, lo, -1):
        ...
        if (y0 > level) and (y1 <= level):
            ...
            if denom == 0:
                ...
                frac = (level - y0) / denom  # between 0 and 1 ideally
...
# recomputing LE timings, (looking back and commenting this i actually dont think this step is necessary oops) 

def dt_from_pairs_LE(pairs_use, thr_A, thr_G=None, search_back=80):
    ...
    for (i, a_pk, g_pk, a_amp, g_amp) in pairs_use:
        ...
        tA = leading_edge_time(a_V, a_pk, thr_A, dt_ns, search_back=search_back)
        tG = leading_edge_time(g_V, g_pk, thr_G, dt_ns, search_back=search_back)
...
def fit_prompt_gaussian(dt_ns, peak_lo=-10, peak_hi=25, peak_bins=200, tmin=-100, tmax=300):
    ...
    if y.size < 5 or np.sum(y) <= 0:
        raise RuntimeError("Not enough prompt-peak statistics to fit.")
...
# thresholds to scan over, from 1mV to 50mV
thr_grid = np.arange(0.001, 0.05, 0.0005)   # volts

scan = []  

pairs_use = pairs   # can limit for quicker testing

# loop over fractions
for thr in thr_grid:
    dt_arr, a_amp_arr, g_amp_arr, n_fail = dt_from_pairs_LE(
        pairs_use,
        thr_A=thr,
        thr_G=thr,
        search_back=80
    ...
    if n_ok < 500:
        scan.append((thr, n_ok, n_fail, np.nan, np.nan))
        continue

    mu, fwhm = prompt_fwhm_score(dt_arr, peak_lo=-10, peak_hi=25)
    scan.append((thr, n_ok, n_fail, mu, fwhm))

scan = np.asarray(scan, dtype=float)

n_max = np.nanmax(scan[:, 1])
keep = scan[:, 1] >= 0.90 * n_max
cand = scan[keep]
...
best_thr, best_n, best_fail, best_mu, best_fwhm = best

# print values
print("Best LE threshold by prompt FWHM:")
print(f"  thr = {best_thr:.3f} V")
...
plt.scatter(scan[:, 0], scan[:, 4], color = 'black', label = 'Data')
plt.axvline(0.004, linestyle = '--', label = 'LE Threshold', color = 'r')
plt.xlabel("LE threshold, absolute value (V)", fontsize=16)
plt.ylabel("Prompt peak FWHM (ns)", fontsize=16)
plt.title("Leading-edge threshold optimisation", fontsize=18)
plt.grid(alpha=0.3)
plt.tight_layout()
plt.legend(fontsize = 15)
plt.savefig('LEthreshold.png')
plt.show()

# number of events that pass
plt.figure(figsize=(8,5))
plt.scatter(scan[:, 0], scan[:, 1], color = 'black')
plt.axvline(0.005, linestyle = '--', label = 'LE Threshold', color = 'r')
plt.xlabel("LE threshold (V)", fontsize=16)
plt.ylabel("Accepted events", fontsize=16)
plt.title("Accepted coincidences vs LE threshold", fontsize=18)
plt.grid(alpha=0.3)
plt.tight_layout()
plt.legend(fontsize = 15)
plt.show()
...
dt_best, a_best, g_best, n_fail_best = dt_from_pairs_LE(
    pairs,
    thr_A=best_thr,   # or best_thrA
    thr_G=best_thr,   # or best_thrG
    search_back=80
)

print("Final LE timing entries:", dt_best.size)
print("Failed crossings:", n_fail_best)
